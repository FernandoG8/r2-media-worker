...
describe('isAuthorized [U-W-06]', () => {
  it('retorna true con X-API-Key correcto', async () => {
    const req = makeRequest('super-secret-key');
    const env = makeEnv('super-secret-key');
    await expect(isAuthorized(req, env)).resolves.toBe(true);
  });

  it('retorna false con X-API-Key incorrecto', async () => {
    const req = makeRequest('wrong-key');
    const env = makeEnv('super-secret-key');
    await expect(isAuthorized(req, env)).resolves.toBe(false);
  });

  it('retorna false cuando X-API-Key esta ausente', async () => {
    const req = makeRequest();
    const env = makeEnv('super-secret-key');
    await expect(isAuthorized(req, env)).resolves.toBe(false);
  });

  it('retorna false cuando X-API-Key es cadena vacia', async () => {
    const req = makeRequest('');
    const env = makeEnv('super-secret-key');
    await expect(isAuthorized(req, env)).resolves.toBe(false);
  });
   ...
   */
  it('rechaza env.API_SECRET vacío y header vacío', async () => {
    const env = { API_SECRET: '' } as unknown as Env;
    const request = new Request('https://example.com', {
      headers: { 'X-API-Key': '' },
    });
    await expect(isAuthorized(request, env)).resolves.toBe(false);
  });

  it('es case-sensitive en el valor de la key', async () => {
    const req = makeRequest('ABC123');
    const env = makeEnv('abc123');
    await expect(isAuthorized(req, env)).resolves.toBe(false);
  });

  it('trata el nombre del header como case-insensitive (estandar HTTP)', async () => {
